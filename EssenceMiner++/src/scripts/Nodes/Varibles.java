...
public class Varibles extends Node {
	...
	public void execute() {
		...
		Miner.path = PathFinding.generatePath(Player.getPosition(),Game.getDestination(),false);
		...
		Miner.CurrentXP = Skills.getXP(SKILLS.MINING);
		 ...
		 Miner.minutes = TimeUnit.SECONDS.toMinutes(Miner.RunTime - TimeUnit.HOURS.toSeconds(Miner.hours));
		 Miner.seconds = Miner.RunTime - (TimeUnit.HOURS.toSeconds(Miner.hours) + TimeUnit.MINUTES.toSeconds(Miner.minutes));
		 Miner.MinedOresHour = 0;
		 Miner.Profit = Miner.MinedOres * new ZybezItem("Rune Essence").getAverage();
		
		 if(Miner.Profit > 0){Miner.ProfitHour = (int) ((Miner.Profit * 3600) / Miner.RunTime);}
		 
		if(Miner.MinedOres > 0){Miner.MinedOresHour = (int) ((Miner.MinedOres * 3600) / Miner.RunTime);}
		
		if(Miner.GainedXP > 0){Miner.GainedXPHour = (int) ((Miner.GainedXP * 3600) / Miner.RunTime);}
